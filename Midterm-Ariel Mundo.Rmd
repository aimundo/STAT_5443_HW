...
```{r, PROBLEM 1}
curve(((1+sqrt(x))/(2*sqrt(x)))*exp(-(x/2+sqrt(x))),from=0, to=8) #plotting the function to determine appropiate envelope
curve(8*dnorm(x,0,2),add=TRUE,col="red")
```
...
while (accept<n){
    y<-runif(1,0,1) #get one random number between 0 and pi/2 as the value to input in f(x)
    u<-rexp(1,1)  #another random number between 0 and 1 to compare
    if (y<((1+sqrt(y))/(2*sqrt(y)))*exp(-(y/2+sqrt(y)))/M/dexp(1,1)){  #if the value of M*u is less or equal than the target distribution, then
        
        accept<-accept+1 #increase the counter by 1 to be able to continue storing in final_sample
        final_sample[accept]<-u #store the random number that was evaluated in f(x)  
    }
}
p<-seq(from=0,to=8,0.01)
w<-((1+sqrt(p))/(2*sqrt(p)))*exp(-(p/2+sqrt(p)))
